...
public class BinarySearchTreeWithDups<T extends Comparable<? super T>> extends
	...
	public ArrayList<T> getAllEntries(T searchVal) {
		...
		Iterator<T> inorderIterator = getInorderIterator();

		boolean pastSearch = false;

		while(inorderIterator.hasNext() && !pastSearch) {
			T currentData = inorderIterator.next();

			int compareVal = searchVal.compareTo(currentData);

			if(compareVal == 0) {
				allEntries.add(currentData);
			}
	...
	private T addEntry(T newEntry) {
		...
		while (!found) {
			...
			if (comparison == 0) { // newEntry matches currentEntry;
									// add duplicate into tree
				found = true;
				result = newEntry;
				BinaryNodeInterface<T> rChild = currentNode.getRightChild();
